/* Nixie Tube Driver V3 - Example 3: Four Nixie Tubes
 *
 * This program drives four nixie tubes connected to the Nixie Tube Driver V3
 ...
 *
 * Each nixie tube has 10 cathodes (digits 0-9). Four cathodes are active
 * at a time (one per tube), so four bits in the 40-bit frame are set to 1.
 *
 * Tube 1 is connected to outputs K01-K10.
 * Tube 2 is connected to outputs K11-K20.
 * Tube 3 is connected to outputs K21-K30.
 * Tube 4 is connected to outputs K31-K40.
 *
 * This example cycles through digits 0-9 on all four tubes simultaneously
...
// ---------------------------------------------------------------------------
// Cycle through digits 0-9 on all four tubes simultaneously, one per second
// ---------------------------------------------------------------------------
void loop()
{
  for (uint8_t digit = 0; digit < 10; digit++)
  {
  //nixieDisplay(tube4, tube3, tube2, tube1);
...
// ---------------------------------------------------------------------------
// nixieDisplay() - display a digit (0-9) on each of the four nixie tubes
//
// Sends a 40-bit frame to the controller shift registers chain.
// Four bits are set to 1 (one active cathode per tube), all other bits are 0.
// ---------------------------------------------------------------------------
void nixieDisplay(uint8_t digit4, uint8_t digit3, uint8_t digit2, uint8_t digit1)
{
  uint8_t activeBit1 = nixieTube1[digit1];
  uint8_t activeBit2 = nixieTube2[digit2];
  uint8_t activeBit3 = nixieTube3[digit3];
  uint8_t activeBit4 = nixieTube4[digit4];
  ...
  {
    digitalWrite(DIN_PIN, (i == activeBit1 || i == activeBit2 ||
                           i == activeBit3 || i == activeBit4) ? HIGH : LOW);
